...
# This project inherits minecraft_version from the Standards build that drives it, so read it from
# there too when nothing is passed — a copy here would be a second place for it to drift.
MC_VERSION="${2:-$(sed -n 's/^minecraft_version=\(.*\)$/\1/p' \
    "$HERE/gradle.properties" "$HERE/../SableCraft-Standards/gradle.properties" 2>/dev/null | head -1)}"
CHANGELOG_FILE="${3:-}"
...
# every 26.x jar - those compile against Java 25, and the file page told people 21 would do.
# Nothing failed, because CurseForge accepts whatever tag it is handed. Fixed in Standards first;
# this copy had the same bug and would have mislabelled every Factions 26.x file the same way.
#
...
# "Factions ReForged 1.0.0 / MC 1.21.11" reads far better in the file list than the raw filename. The mod
...
MOD_VERSION="$(basename "$JAR" .jar | sed -n 's/^factions-\(.*\)+mc.*$/\1/p')"
if [ -n "$MOD_VERSION" ]; then
    DISPLAY_NAME="Factions ReForged $MOD_VERSION / MC $MC_VERSION"
else
